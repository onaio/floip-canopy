...
startFlows() {
  ...
  then 
    ...
    controllerServiceId=$(curl -X GET $NIFI_BASE_URL/nifi-api/flow/process-groups/root/controller-services | jq ".controllerServices[0].id" -r)
    ...
    curl -X PUT -H 'Content-Type: application/json' -d '{"id":"root","state":"RUNNING"}' $NIFI_BASE_URL/nifi-api/flow/process-groups/root
    fi
}
